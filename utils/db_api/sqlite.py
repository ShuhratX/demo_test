...
"""
    ...
    def select_all_info(self):
        sql = """
        SELECT region, COUNT(*) AS Shikoyatlar
        FROM Information GROUP BY region ORDER BY COUNT(*) DESC;
        """
        return self.execute(sql, fetchall=True)


    def add_info(self, region: str, phone: str = None, language: str = 'uz'):
        sql = """
                INSERT INTO Information(region, phone, language) VALUES(?, ?, ?)
                """
        self.execute(sql, parameters=(region, phone, language), commit=True)
